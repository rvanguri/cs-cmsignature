...
| `download_heartmap_scp3689.py` | 01 | SCP bulk-download curl | atlas h5ad | paste SCP `auth_code` curl (non-fatal if absent) |
| `inspect_heartmap.py` | manual | `HeartMap_V1.0.h5ad` (~80 GB) | structure printout + optional slim reference | run backed first; `--keep-layer` needs a high-mem `fn_*` node |
...
| `run_heartmap_projection.py` | 09 | integrated h5ad + atlas | composition + niche TSVs | load HeartMap's trained model if shipped (non-fatal if atlas absent) |
...
- `obs` column names assumed: `study`, `disease`, `individual`, `anatomy`, `sex`,
  `predicted_cell_type`. If your h5ads differ, set the `--*_col` flags (DE/gates/projection expose them)
