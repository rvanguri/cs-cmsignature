# Pipeline Plan: Unified scANVI Pipeline: Cardiac Sarcoidosis vs. Mimics

**Target platform: an HPC/SLURM cluster.** This plan describes execution entirely on an HPC/SLURM
cluster with GPFS-style storage. **The science is unchanged**: same datasets, same
uniform-decontamination logic, same two gates (decontamination QC + procurement robustness), same
scANVI integration and pseudobulk DE. The *infrastructure* layer is: Slurm scheduling, Lmod modules,
Singularity containers (no Docker on the cluster), shared parallel storage layout, GPU-partition
CellBender, and controlled-access data staging under your institution's data-governance process.

If you want the underlying scientific rationale (the procurement confound, the TNNI3K/GJB7
correction), see Plan v2.1: it is carried forward verbatim in intent. This document is the
**execution layer**.

---

## 0. Cluster execution model

This plan assumes a Slurm-scheduled HPC cluster with Lmod modules, a conda/mamba analysis env, and
Singularity/Apptainer for CellBender (built or pulled from `docker://`); GPU partitions with
`--gres=gpu:N` for CellBender and scANVI; a shared parallel filesystem exposing persistent
(`/gpfs/data/<lab>`), fast-but-purged scratch (`/gpfs/scratch/<kid>`), and home
(`/gpfs/home/<kid>`) tiers; job arrays (`--array`) running one CellBender task per sample at 4–8
concurrent; and controlled data (dbGaP phs001539 / EGA) staged only after a Data Use Agreement and
institutional data-governance sign-off.
...
1. **Heavy I/O and intermediates live on `/gpfs/scratch`** (flash, fast) but scratch is **purged**
   ...
   finishes. The original v2.1 "persist to `/mnt/shared-workspace/`" maps to **`/gpfs/data/<lab>/cs-cmsignature/`**.
...
```
PROJ=/gpfs/data/<lab>/cs-cmsignature           # persistent project root (deliverables live here)
SCRATCH=/gpfs/scratch/<kid>/cs-cmsignature     # fast, PURGED: intermediates only
...
plus a reference atlas). Keep the 3.7 TB Reichart EGA BAM *out of scope*: we use the CELLxGENE
...
celda(DecontX), DESeq2. The cluster typically provides `miniconda3`/`anaconda3` via Lmod; create the env on
`/gpfs/home` or `/gpfs/data` (not scratch: it'd get purged).

```bash
module load miniconda3            # confirm exact name: module avail conda
mamba env create -f env/environment.yml -p $PROJ/env/conda/cs-cmsignature
```
...
This is the load-bearing change from v2.1. On an HPC cluster, controlled data has a *process*, not just a
download.
...
- A **harmonized reference atlas** (if one is openly downloadable): atlases are usually open even
  when per-study raw is gated. If available, this gives you the **label/signature transfer** path for
  ICM/ARVC/LV-HCM **without** touching controlled raw.

**3.2 Controlled raw (gated): Chaffin & Simonson via dbGaP phs001539; Reichart raw via EGA:**
You indicated you need access guidance, so the typical cluster path is:

1. **Confirm whether you even need controlled raw.** If a harmonized reference atlas is
...
2. **dbGaP phs001539 (controlled):** requires an approved **Data Access Request (DAR)** through your
   ...
   repository key (`.ngc`)**.
3. **Institutional data governance:** controlled-access human genomic data on the cluster must land in a
...
inclusion (integrated 3′ scANVI arm + standalone Liu 5′ co-primary + reference-atlas option) all
carry over exactly. The only cluster-relevant note: build a **single sample manifest TSV**
...
- `model.train(max_epochs=400, early_stopping=True)`; save model + latent + predictions to
  ...
  the cluster, GPU is faster where you have access, so default GPU with CPU fallback.)
...
Extract CMs; pseudobulk per patient (sum → integer). **Pseudobulk DE model**
...
apeglm shrinkage + dupCor + ≥75% concordance + procurement gate. IEG/HSP + cross-study DCM generic-HF
...
set. All v2.1 figures/tables incl. decontamination + procurement-robustness + cross-disease positioning.
`cpu_short`.

---

## 6. Phase 3: Compute strategy mapped to the cluster
...
partition** (if the cluster provides high-mem nodes) rather than chasing memory on a standard node.
...
## 8. Caveats: cluster-specific (in addition to all v2.1 caveats)

1. **All v2.1 scientific caveats stand** (Reichart can't be CellBender-re-run from raw; procurement is
   structurally confounded; the reference atlas has no CS; single-CS-cohort limit; the reference atlas lumps lymphocytes;
   Neyazi GEO has no published labels). None of these are fixed by changing clusters.
2. **Scratch purge risk.** The single biggest cluster-specific failure mode: losing CellBender/DecontX
...
3. **Controlled-data governance is a hard gate, not a formality.** dbGaP phs001539 on the cluster needs an
...
1. **Phase 0 access audit**: fill `env/access_status.md` (atlas-transfer default); email
   your HPC support team if any arm is `raw-reprocess`; verify reference-atlas downloadability.
...
*Generated as an HPC/SLURM adaptation of Plan v2.1. Scientific design, contrasts, gates, and gene
